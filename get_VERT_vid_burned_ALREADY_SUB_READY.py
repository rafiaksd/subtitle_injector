...
def modify_ass_to_center(ass_path: str):
    ...
    for line in lines:
        if line.strip().startswith("[V4+ Styles]"):
            ...
            parts[2] = '22' #font size
            parts[7] = '1'
            parts[16] = '0'
            parts[18] = '5'  # Alignment
            parts[19] = '5'  # MarginL
...
def add_fade_animation_to_ass(file_path, fade_out_ms=100):
    ...
    print("✅ Fade effect added to .ass subtitles")
...
ass_file = "test_sub.ass"

title_text = sanitize_for_ffmpeg(input("Enter Sheikh Name: "))
caption_text = get_caption_text()
bottom_text = sanitize_for_ffmpeg(input("Enter Video Title: "))
output_filename = sanitize_for_ffmpeg(bottom_text.strip()) + ".mp4"

print("👀 Select CROP AREA (use A/D to scroll, C to confirm)...")
crop_x, crop_y, crop_w, crop_h = select_crop_area(video_path)

# Subtitle processing
convert_srt_to_ass(srt_file, ass_file)
modify_ass_to_center(ass_file)
add_fade_animation_to_ass(ass_file)

# Output directory
os.makedirs("burned_videos", exist_ok=True)
final_output = os.path.join("burned_videos", output_filename)

# Perform direct video burn
overlay_and_concatenate(
    video_path, bg_path, crop_x, crop_y, crop_w, crop_h,
...
os.startfile(os.path.abspath("burned_videos"))
